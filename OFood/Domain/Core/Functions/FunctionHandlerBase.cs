...
namespace OFood.Domain.Core.Functions
{
    /// <summary>
    /// 功能信息处理基类
    /// </summary>
        ...
        where TFunction : class, IEntity<Guid>, IFunction, new()
    {
        ...
        {
            FunctionTypeFinder.CheckNotNull(nameof(FunctionTypeFinder));
        ...
        {
            functions.CheckNotNull(nameof(functions));
            ...
            foreach (TFunction item in dbItems.Except(removeItems))
            {
                ...
                {
                    throw new OFoodException($"发现多个“{item.Area}-{item.Controller}-{item.Action}”的功能信息，不允许重名");
                }
                if (function == null)
                {
                    continue;
